...
//    NSData *imgData = UIImageJPEGRepresentation([WFCCUtilities imageCompressForSize:image targetSize:CGSizeMake(1024, 1024)], 0.85);
    [imgData writeToFile:path atomically:YES];
    
    content.localPath = path;
    content.thumbnail = [WFCCUtilities generateThumbnail:image withWidth:120 withHeight:120];
//    content.thumbnail = [WFCCUtilities imageCompressForSize:image targetSize:CGSizeMake(120, 120)];
...
- (WFCCMessagePayload *)encode {
    ...
    payload.searchableContent = @"[IMG]";
    payload.binaryContent = UIImageJPEGRepresentation(self.thumbnail, 0.5);
...
//        self.thumbnail = [WFCCUtilities imageCompressForSize:self.thumbnail targetSize:CGSizeMake(120, 120)];
...
//        _thumbnail = [WFCCUtilities imageCompressForSize:image targetSize:CGSizeMake(120, 120)];
    }
    return _thumbnail;
}

...
#warning 为了处理scrollViewDidEndScrollingAnimation，即使selectedIndex和targetIndex相同，也不能return。
    // self.selectedIndex 已选择的index， targetIndex目标index
...
#warning 目标targetIndex和之前已选择的_selectedIndex相距太远，则禁用动画
            if (labs(targetIndex - _selectedIndex) > 2) {
                [self.contentScrollView setContentOffset:CGPointMake(targetIndex*self.contentScrollView.bounds.size.width, 0) animated:NO];
            }
            else{
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    NSLog(@"JXCategoryBaseView collectionView didSelectItemAtIndexPath");
...
- (CGRect)getTargetCellFrame:(NSInteger)targetIndex
{
    ...
    if (self.dataSource.count > 0) {
        ...
        NSLog(@"error getTargetCellFrame self.dataSource nil");
    }
    
    return CGRectMake(x, 0, 0, self.bounds.size.height);
}

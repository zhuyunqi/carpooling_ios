...
//    [self addSubview:self.emojSwitchBtn];
    ...
    self.textInputView = [[UITextView alloc] initWithFrame:CGRectMake(15, CHAT_INPUT_BAR_PADDING, parentRect.size.width - 30 - CHAT_INPUT_BAR_HEIGHT + CHAT_INPUT_BAR_PADDING, CHAT_INPUT_BAR_ICON_SIZE)];
...
//    [self.voiceInputBtn addTarget:self action:@selector(onTouchUpOutside:) forControlEvents:UIControlEventTouchCancel];
    ...
    if (@available(iOS 13.0, *)) {
        ...
        self.textInputView.backgroundColor = dyColor;
        ...
        UIColor *dyColor2 = [UIColor colorWithDynamicProvider:^UIColor * _Nonnull(UITraitCollection * _Nonnull trainCollection) {
            if ([trainCollection userInterfaceStyle] == UIUserInterfaceStyleLight) {
                return HEXCOLOR(0xdbdbdd);
            }
            else {
                return [UIColor secondarySystemBackgroundColor];
            }
        }];
        self.textInputView.layer.borderColor = dyColor2.CGColor;
        self.layer.borderColor = dyColor2.CGColor;
        ...
        self.textInputView.tintColor = dyColor3;
        self.textInputView.textColor = dyColor3;
        
        UIColor *dyColor4 = [UIColor colorWithDynamicProvider:^UIColor * _Nonnull(UITraitCollection * _Nonnull trainCollection) {
            if ([trainCollection userInterfaceStyle] == UIUserInterfaceStyleLight) {
        ...
        self.inputCoverView.backgroundColor = dyColor4;
...
//        [navi presentViewController:nav animated:YES completion:nil];
    ...
    // contract
        ...
        [self.delegate didSelectContract];
        [self notifyTyping:5];
    }
}

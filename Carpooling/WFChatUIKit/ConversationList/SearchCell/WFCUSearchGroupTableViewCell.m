...
- (void)setGroupSearchInfo:(WFCCGroupSearchInfo *)groupSearchInfo {
    ...
    if (groupSearchInfo.marchType > 0) {
        ...
        if (string == nil) {
            string = [[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@<%@>", kLocalizedTableString(@"user", @"CPLocalizable"), groupSearchInfo.marchedMemberNames[0]]] mutableCopy];
        }

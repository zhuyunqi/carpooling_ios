...
- (void)viewDidLoad {
  ...
  if (!_isModifyPortrait) {
    ...
    [portraitView sd_setImageWithURL:[NSURL URLWithString:groupInfo.portrait] placeholderImage:[UIImage imageNamed:@"messege_no_icon"]];
    ...
    CGFloat width = (PortraitWidth - padding) / numPerRow - padding;
        ...
        for (int i = 0; i < row; i++) {
            ...
            for (int j = 0; j < c; j++) {
                ...
                [imageView sd_setImageWithURL:[NSURL URLWithString:user.portrait] placeholderImage:[UIImage imageNamed:@"messege_no_icon"]];

//
//  WFCUStrangerMessageController.m
//  Carpooling
//
...
@interface WFCUStrangerMessageController () <UITextFieldDelegate, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource, UINavigationControllerDelegate, WFCUMessageCellDelegate, AVAudioPlayerDelegate, WFCUChatInputBarDelegate, SDPhotoBrowserDelegate, UIGestureRecognizerDelegate>
...
@property(nonatomic, strong) UIView *matchingTopHeaderView;
@property(nonatomic, strong) UIButton *addFriendBtn;
@property(nonatomic, strong) UIButton *setupContractBtn;
@end

@implementation WFCUStrangerMessageController
...
- (void)initializedSubViews {
    ...
    if (self.showMatchingTopHeaderView) {
    ...
    frame.origin.y += kStatusBarAndNavigationBarHeight + self.matchingTopHeaderView.frame.size.height;
    frame.size.height -= (kTabbarSafeBottomMargin + kStatusBarAndNavigationBarHeight + self.matchingTopHeaderView.frame.size.height);
    self.backgroundView = [[UIView alloc] initWithFrame:frame];
    
    if (self.showMatchingTopHeaderView) {
        [self.view insertSubview:self.backgroundView belowSubview:self.matchingTopHeaderView];
    }
    else {
...
//                    WFCUStrangerMessageController *mvc = [[WFCUStrangerMessageController alloc] init];
...
- (void)didAddFriend:(WFCUMessageCellBase *)cell withModel:(WFCUMessageModel *)model withAgree:(BOOL)agree{
    NSLog(@"WFCUStrangerMessageController didAddFriend agree:%d", agree);
    ...
    if (agree) {
        ...
        NSLog(@"WFCUStrangerMessageController didAddFriend 忽略好友请求");
        WFCCAddFriendMessageContent *content = (WFCCAddFriendMessageContent*)model.message.content;
        content.status = 2;
...
- (void)didDealContract:(WFCUMessageCellBase *)cell withModel:(WFCUMessageModel *)model withAgree:(BOOL)agree{
    ...
    if (agree) {
        NSLog(@"WFCUStrangerMessageController didDealContract 同意合约");
        content.status = 1;
        ...
        NSLog(@"WFCUStrangerMessageController didDealContract 拒绝合约");
        content.status = 2;
...
- (void)requestCollectAddress:(WFCUMessageCellBase*)cell{
    ...
    [[NetworkManager shareNetworkManager] POSTUrl:[NSString stringWithFormat:@"%@/api/address/v1/collectAddress", BaseURL] parameters:param.mutableCopy success:^(id responseObject) {
        NSLog(@"WFCUStrangerMessageController requestCollectAddress responseObject:%@", responseObject);
        if (responseObject) {
            if ([[responseObject valueForKey:@"code"] integerValue] == 200) {
                NSLog(@"WFCUStrangerMessageController requestCollectAddress 成功");
                content.status = 1;
                ...
                NSLog(@"WFCUStrangerMessageController requestCollectAddress 失败");
            }
        }
        else {
            NSLog(@"WFCUStrangerMessageController requestCollectAddress 失败");
        ...
        NSLog(@"WFCUStrangerMessageController requestCollectAddress error:%@, judgeCode:%lu", [error localizedDescription], (unsigned long)judgeCode);
    }];
}


#pragma mark - set up headerview
- (void)setupHeaderView{
    CGFloat headerHeight = 0;
    UIView *headerView = [[UIView alloc] initWithFrame:CGRectZero];
    headerView.backgroundColor = RGBA(120, 202, 195, 1);
    [self.view addSubview:headerView];
    self.matchingTopHeaderView = headerView;
    
    NSString *str1 = kLocalizedTableString(@"Add Friend", @"CPLocalizable");
    CGSize size1 = [str1 sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14.f]}];
    CGFloat width1 = size1.width+20;
    
    
    UIButton *btn1 = [UIButton buttonWithType:UIButtonTypeCustom];
    btn1.backgroundColor = RGBA(253, 228, 71, 1);
    btn1.tag = 10015;
    btn1.frame = CGRectMake(15, 5, width1, 38);
    //    btn1.layer.borderWidth = 1;
    [btn1 setTitle:str1 forState:UIControlStateNormal];
    ...
    //    btn1.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
    [btn1 addTarget:self action:@selector(addFriendAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.matchingTopHeaderView addSubview:btn1];
    headerHeight += 38;
    
    
    NSString *str2 = kLocalizedTableString(@"Msg Contract", @"CPLocalizable");
    CGSize size2 = [str2 sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14.f]}];
    CGFloat width2 = size2.width+40;
    
    UIButton *btn2 = [UIButton buttonWithType:UIButtonTypeCustom];
    btn2.backgroundColor = [UIColor whiteColor];
    btn2.tag = 10016;
    btn2.frame = CGRectMake(kSCREENWIDTH-width2-15, 5, width2, 38);
    [btn2 setTitle:str2 forState:UIControlStateNormal];
    btn2.titleLabel.font = [UIFont systemFontOfSize:14.f];
    btn2.layer.cornerRadius = btn1.frame.size.height/2;
    btn2.layer.masksToBounds = YES;
    [btn2 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    btn2.contentEdgeInsets = UIEdgeInsetsMake(0, 25, 0, 0);
    [btn2 addTarget:self action:@selector(prepareSetupAlertView:) forControlEvents:UIControlEventTouchUpInside];
    [self.matchingTopHeaderView addSubview:btn2];
    
    UIImageView *imgV1 = [[UIImageView alloc] initWithFrame:CGRectMake(CGRectGetMinX(btn2.frame)+10, 5+(38-18)/2, 18, 18)];
    imgV1.image = [UIImage imageNamed:@"messege_send_contract"];
    [self.matchingTopHeaderView addSubview:imgV1];
    
    
    UILabel *lbl1 = [[UILabel alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(btn1.frame)+5, kSCREENWIDTH-30, 20)];
    ...
    if (@available(iOS 8.2, *)) {
        lbl1.font = [UIFont systemFontOfSize:15.f weight:UIFontWeightMedium];
    } else {
        // Fallback on earlier versions
        lbl1.font = [UIFont boldSystemFontOfSize:15.f];
    }
    [self.matchingTopHeaderView addSubview:lbl1];
    headerHeight += 20;
    
    
    UILabel *lbl2 = [[UILabel alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(lbl1.frame)+5, 45, 20)];
    lbl2.text = kLocalizedTableString(@"From", @"CPLocalizable");
    lbl2.textColor = [UIColor whiteColor];
    if (@available(iOS 8.2, *)) {
        lbl2.font = [UIFont systemFontOfSize:15.f weight:UIFontWeightMedium];
    } else {
        // Fallback on earlier versions
        lbl2.font = [UIFont boldSystemFontOfSize:15.f];
    }
    [self.matchingTopHeaderView addSubview:lbl2];
    headerHeight += 20;
    
    NSString *address1 = @"";
    if(self.scheduleMJModel){
        address1 = self.scheduleMJModel.fromAddressVo.address;
    }
    else{
        address1 = self.contractMJModel.fromAddressVo.address;
    }
    
    CGSize size3 = W_GET_STRINGSIZE(address1, kSCREENWIDTH-65, MAXFLOAT, [UIFont systemFontOfSize:15.f]);
    CGFloat height3 =  size3.height;
    
    UILabel *lbl3 = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(lbl2.frame), CGRectGetMinY(lbl2.frame), kSCREENWIDTH-65, height3)];
    lbl3.numberOfLines = 0;
    lbl3.text = address1;
    lbl3.textColor = [UIColor whiteColor];
    lbl3.font = [UIFont systemFontOfSize:15.f];
    [self.matchingTopHeaderView addSubview:lbl3];
    headerHeight += height3;
    
    
    UILabel *lbl4 = [[UILabel alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(lbl3.frame)+8, 40, 20)];
    lbl4.text = kLocalizedTableString(@"To", @"CPLocalizable");
    lbl4.textColor = [UIColor whiteColor];
    if (@available(iOS 8.2, *)) {
        lbl4.font = [UIFont systemFontOfSize:15.f weight:UIFontWeightMedium];
    } else {
        // Fallback on earlier versions
        lbl4.font = [UIFont boldSystemFontOfSize:15.f];
    }
    [self.matchingTopHeaderView addSubview:lbl4];
    headerHeight += 20;
    
    NSString *address2 = @"";
    if(self.scheduleMJModel){
        address2 = self.scheduleMJModel.toAddressVo.address;
    }
    else{
        address2 = self.contractMJModel.toAddressVo.address;
    }
    CGSize size5 = W_GET_STRINGSIZE(address2, kSCREENWIDTH-65, MAXFLOAT, [UIFont systemFontOfSize:15.f]);
    CGFloat height5 =  size5.height;
    
    UILabel *lbl5 = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(lbl2.frame), CGRectGetMinY(lbl4.frame), kSCREENWIDTH-65, height5)];
    lbl5.numberOfLines = 0;
    lbl5.text = address2;
    lbl5.textColor = [UIColor whiteColor];
    lbl5.font = [UIFont systemFontOfSize:15.f];
    [self.matchingTopHeaderView addSubview:lbl5];
    headerHeight += height5-3;
    
    self.matchingTopHeaderView.frame = CGRectMake(0, kNAVIBARANDSTATUSBARHEIGHT, kSCREENWIDTH, headerHeight);
}
...
//
//                            dispatch_async(dispatch_get_main_queue(), ^{
//                                NSArray *array  = [[WFCCIMService sharedWFCIMService] getOutgoingFriendRequest];
//                                for (WFCCFriendRequest *request in array) {
//                                    if ([request.target isEqualToString:self.conversation.target]) {
////                                        request.status = 1;
//                                        NSLog(@"好友请求失败 request.status:%d", request.status);
//                                        break;
//                                    }
//                                }
//
...
- (void)prepareSetupAlertView:(id)sender{
    if (nil == [[NSUserDefaults standardUserDefaults] valueForKey:kUserLoginAccount]) {
        [self.view endEditing:YES];
        UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
        CPUserLoginVC *userLoginVC = [storyboard instantiateViewControllerWithIdentifier:@"CPUserLoginVC"];
    ...
    self.selectedContractType = 0; // 默认短期合约
    [self setupAlertView];
}
...
- (void)requestAcceptContractByContractId:(NSUInteger)contractId complete:(void (^)(BOOL success))callbackBlock{
    ...
    [[NetworkManager shareNetworkManager] POSTUrl:[NSString stringWithFormat:@"%@/api/contract/v1/acceptContract", BaseURL] parameters:@{@"contractId":[NSNumber numberWithInteger:contractId]}.mutableCopy success:^(id responseObject) {
        ...
        NSLog(@"WFCUStrangerMessageController requestAcceptContractByContractId responseObject:%@", responseObject);
        if (responseObject) {
            if ([[responseObject valueForKey:@"code"] integerValue] == 200) {
                // postnoti
            ...
            callbackBlock(NO);
            NSLog(@"WFCUStrangerMessageController requestAcceptContractByContractId 失败");
        ...
        callbackBlock(NO);
        [SVProgressHUD showErrorWithStatus:[error localizedDescription]];
        NSLog(@"WFCUStrangerMessageController requestAcceptContractByContractId error:%@, judgeCode:%lu", [error localizedDescription], (unsigned long)judgeCode);
    }];
}
...
- (UNNotificationRequest *)createNotificationRequestWithWeekday:(NSInteger)weekday
                                                     ...
                                                     contractId:(NSInteger)contractId contractType:(NSUInteger)contractType
                                                      beginTime:(NSString*)beginTime
                                                        ...
                                                        endDate:(NSString*)endDate
                                                        weekNum:(NSString*)weekNum {
    NSLog(@"WFCUStrangerMessageController createNotificationRequestWithWeekday identifier:%@", identifier);
...
- (UNNotificationRequest *)createNotificationRequestWithDate:(NSDate*)date
                                                ...
                                                contractType:(NSUInteger)contractType
                                                   beginTime:(NSString*)beginTime
                                                     weekNum:(NSString*)weekNum {
    NSLog(@"WFCUStrangerMessageController createNotificationRequestWithDate identifier:%@", identifier);
...
//    NSLog(@"WFCUStrangerMessageController cancelNotificationWithIdentifier identifier:%@", identifier);
//
//    if (@available(iOS 10.0, *)) {

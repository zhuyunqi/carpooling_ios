...
+ (CGFloat)bubbleWidth {
    return ([UIScreen mainScreen].bounds.size.width - 2*Portrait_Size - 2*Portrait_Padding_Left - 2*Portrait_Padding_Right) - 30;
}
...
- (void)setModel:(WFCUMessageModel *)model {
  ...
  if (model.message.direction == MessageDirection_Send) {
    ...
    CGSize size = [self.class sizeForClientArea:model withViewWidth:[WFCUMessageCell clientAreaWidth]];
      self.bubbleView.image = [UIImage imageNamed:@"icon_qipao1"];
...
//      UIImage *image = self.bubbleView.image;
    ...
    CGSize size = [self.class sizeForClientArea:model withViewWidth:[WFCUMessageCell clientAreaWidth]];
      self.bubbleView.image = [UIImage imageNamed:@"icon_qipao2"];
...
//      UIImage *image = self.bubbleView.image;
      UIEdgeInsets imageInsets = UIEdgeInsetsMake(SSChatAirTop, SSChatAirLRB, SSChatAirBottom, SSChatAirLRS);
      self.bubbleView.image = [self.bubbleView.image
                               resizableImageWithCapInsets:imageInsets];
  }
...
- (UIImageView *)portraitView {
  if (!_portraitView) {
    ...
    [_portraitView setImage:[UIImage imageNamed:@"messege_no_icon"]];

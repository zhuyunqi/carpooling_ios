...
- (void)setConversation:(WFCCConversation *)conversation {
    ...
    if (conversation.type == Single_Type) {
        ...
        if (userInfo) {
            ...
            name = [NSString stringWithFormat:@"%@<%@>", kLocalizedTableString(@"user", @"CPLocalizable"), conversation.target];
        }
        [self.portraitImageView sd_setImageWithURL:[NSURL URLWithString:portrait] placeholderImage:[UIImage imageNamed:@"messege_no_icon"]];

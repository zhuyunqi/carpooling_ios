...
+ (NSString *)formatTimeLabel:(int64_t)timestamp {
    ...
    if (days == curDays) {
        ...
        return kLocalizedTableString(@"Yday", @"CPLocalizable");
    } else {
        ...
        if (weeks == curWeeks) {
            switch (weekDays) {
                case 1:
                    return kLocalizedTableString(@"SundayShort", @"CPLocalizable");
                    break;
                case 2:
                    return kLocalizedTableString(@"MondayShort", @"CPLocalizable");
                    break;
                case 3:
                    return kLocalizedTableString(@"TuesdayShort", @"CPLocalizable");
                    break;
                case 4:
                    return kLocalizedTableString(@"WednesdayShort", @"CPLocalizable");
                    break;
                case 5:
                    return kLocalizedTableString(@"ThursdayShort", @"CPLocalizable");
                    break;
                case 6:
                    return kLocalizedTableString(@"FridayShort", @"CPLocalizable");
                    break;
                case 7:
                    return kLocalizedTableString(@"SaturdayShort", @"CPLocalizable");
                    break;
                    
                default:
                    break;
            }
            return [NSString stringWithFormat:@"%@%ld", kLocalizedTableString(@"Week", @"CPLocalizable"), (long)weekDays];
        } else {
...
+ (NSString *)formatTimeDetailLabel:(int64_t)timestamp {
    ...
    if (days == curDays) {
        ...
        return [NSString stringWithFormat:@"%@ %@", kLocalizedTableString(@"Yday", @"CPLocalizable"), time];
    } else {
        NSInteger weeks = [calendar component:NSCalendarUnitWeekOfYear fromDate:date];
        NSInteger curWeeks = [calendar component:NSCalendarUnitWeekOfYear fromDate:current];

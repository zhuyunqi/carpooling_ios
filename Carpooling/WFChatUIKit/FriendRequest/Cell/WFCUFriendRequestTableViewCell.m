...
- (void)setFriendRequest:(WFCCFriendRequest *)friendRequest {
    ...
    [self.portraitView sd_setImageWithURL:[NSURL URLWithString:userInfo.portrait]  placeholderImage: [UIImage imageNamed:@"messege_no_icon"]];
    ...
    if (friendRequest.status == 0 && !expired) {
        [self.acceptBtn setTitle:kLocalizedTableString(@"Accept", @"CPLocalizable") forState:UIControlStateNormal];
        [self.acceptBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [self.acceptBtn setBackgroundColor:[UIColor greenColor]];
        [self.acceptBtn setEnabled:YES];
    } else if (friendRequest.status == 1) {
        [self.acceptBtn setTitle:kLocalizedTableString(@"Already Agree friend", @"CPLocalizable") forState:UIControlStateNormal];
        ...
        [self.acceptBtn setTitle:kLocalizedTableString(@"Reject", @"CPLocalizable") forState:UIControlStateNormal];
        [self.acceptBtn setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
        [self.acceptBtn setBackgroundColor:self.backgroundColor];
        [self.acceptBtn setEnabled:NO];
    } else { //expired
        [self.acceptBtn setTitle:kLocalizedTableString(@"Expired", @"CPLocalizable") forState:UIControlStateNormal];
        [self.acceptBtn setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
        [self.acceptBtn setBackgroundColor:self.backgroundColor];
        [self.acceptBtn setEnabled:NO];
    }

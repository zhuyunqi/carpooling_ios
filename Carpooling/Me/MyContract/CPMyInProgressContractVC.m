...
@interface CPMyInProgressContractVC ()<CPMyInProgressContractCell1Delegate>
...
@implementation CPMyInProgressContractVC
...
- (void)viewDidLoad {
    ...
    [SVProgressHUD show];
    [self requestMyInProgressContractByCurrentIndex:_currIndex];
}
...
//    else if (ridingStatus == 1) {
    ...
    _currIndex = 1;
    [self requestMyInProgressContractByCurrentIndex:_currIndex];
    NSLog(@"CPMyInProgressContractVC needRefreshArriveState");
}
...
- (NSInteger)numberOfSectionsInTableView:(UITableView*)tableView {
    NSLog(@"CPMyInProgressContractVC _dataSource.count:%lu", (unsigned long)_dataSource.count);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    contractDetailVC.passValueblock = ^(BOOL cancel) {
        self.currIndex = 1;
        [self requestMyInProgressContractByCurrentIndex:self.currIndex];
    };
...
- (void)requestMyInProgressContractByCurrentIndex:(NSUInteger)index{
    ...
    [[NetworkManager shareNetworkManager] POSTUrl:[NSString stringWithFormat:@"%@/api/contract/v1/getContractList.json", BaseURL] parameters:param success:^(id responseObject) {
        ...
        NSLog(@"CPMyInProgressContractVC requestMyInProgressContract responseObject:%@", responseObject);
        if (responseObject) {
        ...
        [SVProgressHUD dismiss];
        [weakSelf.tableView.mj_footer endRefreshing];
        NSLog(@"CPMyInProgressContractVC  requestMyInProgressContract error:%@, judgeCode:%lu", [error localizedDescription], (unsigned long)judgeCode);
    }];
}
...
- (void)requestCancelContractById:(NSUInteger)index{
    ...
    [[NetworkManager shareNetworkManager] POSTUrl:[NSString stringWithFormat:@"%@/api/contract/v1/cancelContract", BaseURL] parameters:@{@"contractId":[NSNumber numberWithInteger:contractMJModel.dataid]}.mutableCopy success:^(id responseObject) {
        [SVProgressHUD dismiss];
        NSLog(@"CPMyInProgressContractVC requestCancelContractByID responseObject:%@", responseObject);
        WS(weakSelf);
        if (responseObject) {
            ...
            NSLog(@"CPMyInProgressContractVC requestCancelContractByID 失败");
        }
        
        
    } failure:^(NSError *error, ParamtersJudgeCode judgeCode) {
        [SVProgressHUD showErrorWithStatus:[error localizedDescription]];
        NSLog(@"CPMyInProgressContractVC requestCancelContractByID error:%@, judgeCode:%lu", [error localizedDescription], (unsigned long)judgeCode);
    }];
}
    ...
    [[NetworkManager shareNetworkManager] POSTUrl:[NSString stringWithFormat:@"%@/api/contract/v1/confirmArrive", BaseURL] parameters:@{@"contractId":[NSNumber numberWithInteger:contractMJModel.dataid]}.mutableCopy success:^(id responseObject) {
        ...
        if (responseObject) {
            if ([[responseObject valueForKey:@"code"] integerValue] == 200) {
                ...
                contractMJModel.onCarTimestamp = [[dict valueForKey:@"onCarTimestamp"] integerValue];
                contractMJModel.confirmArriveTimestamp = [[dict valueForKey:@"confirmArriveTimestamp"] integerValue];
...
- (void)requestIfUserIsLogin{
    ...
    [[NetworkManager shareNetworkManager] POSTUrl:[NSString stringWithFormat:@"%@/api/auth/v1/islogin", BaseURL] parameters:nil success:^(id responseObject) {
        [SVProgressHUD dismiss];
        NSLog(@"CPMyInProgressContractVC requestIfUserIsLogin responseObject:%@", responseObject);
        if (responseObject) {
            if ([[responseObject valueForKey:@"code"] integerValue] == 200) {
                NSString *account = [[NSUserDefaults standardUserDefaults] valueForKey:kUserLoginAccount];
                NSLog(@"CPMyInProgressContractVC requestIfUserIsLogin 拼车号:%@ has login ", account);
                
                NSString *savedToken = [[NSUserDefaults standardUserDefaults] stringForKey:@"savedToken"];
                NSString *savedUserId = [[NSUserDefaults standardUserDefaults] stringForKey:@"savedUserId"];
                
                NSLog(@"CPMyInProgressContractVC savedUserId:%@, [WFCCNetworkService sharedInstance].userId:%@", savedUserId, [WFCCNetworkService sharedInstance].userId);
            ...
            NSLog(@"CPMyInProgressContractVC requestIfUserIsLogin 失败");
        }
        
    } failure:^(NSError *error, ParamtersJudgeCode judgeCode) {
        [SVProgressHUD showErrorWithStatus:[error localizedDescription]];
        NSLog(@"CPMyInProgressContractVC requestIfUserIsLogin error:%@, judgeCode:%lu", [error localizedDescription], (unsigned long)judgeCode);
    }];
}

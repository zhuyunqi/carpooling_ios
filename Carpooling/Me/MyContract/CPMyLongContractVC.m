...
@interface CPMyLongContractVC ()<CPMyInProgressContractCell1Delegate>
...
@implementation CPMyLongContractVC
...
- (void)viewDidLoad {
    ...
    [SVProgressHUD show];
    [self requestMyLongContractByCurrentIndex:_currIndex];
}
...
- (NSInteger)numberOfSectionsInTableView:(UITableView*)tableView {
    NSLog(@"CPMyLongContractVC _dataSource.count:%lu", (unsigned long)_dataSource.count);
...
- (void)requestMyLongContractByCurrentIndex:(NSUInteger)index{
    ...
    NSLog(@"CPMyLongContractVC requestMyLongContractByCurrentIndex self.currIndex:%lu", self.currIndex);
    ...
    [[NetworkManager shareNetworkManager] POSTUrl:[NSString stringWithFormat:@"%@/api/contract/v1/getContractList.json", BaseURL] parameters:param success:^(id responseObject) {
        ...
        NSLog(@"CPMyLongContractVC requestMyLongContractByCurrentIndex responseObject:%@", responseObject);
        if (responseObject) {
            ...
            if (masterModel.code == 200) {
                ...
                NSLog(@"CPMyLongContractVC requestMyLongContractByCurrentIndex requestRegister 失败");
            }
        }
        else {
            NSLog(@"CPMyLongContractVC requestMyLongContractByCurrentIndex requestRegister 失败");
        ...
        [SVProgressHUD dismiss];
        [weakSelf.tableView.mj_footer endRefreshing];
        NSLog(@"CPMyLongContractVC  requestMyLongContractByCurrentIndex error:%@, judgeCode:%lu", [error localizedDescription], (unsigned long)judgeCode);
    }];
}
...
- (void)contractCell1DetailAction:(NSIndexPath *)indexPath{
    ...
    contractDetailVC.passValueblock = ^(BOOL cancel) {
        self.currIndex = 1;
        [self requestMyLongContractByCurrentIndex:self.currIndex];
    };
    CPContractMJModel *contractMJModel = [self.dataSource objectAtIndex:indexPath.section];
    contractDetailVC.contractId = contractMJModel.dataid;
    [[Utils getSupreViewController:self.view].navigationController pushViewController:contractDetailVC animated:YES];
...
- (void)requestCancelContractById:(NSUInteger)index{
    ...
    [[NetworkManager shareNetworkManager] POSTUrl:[NSString stringWithFormat:@"%@/api/contract/v1/cancelContract", BaseURL] parameters:@{@"contractId":[NSNumber numberWithInteger:contractMJModel.dataid]}.mutableCopy success:^(id responseObject) {
        [SVProgressHUD dismiss];
        NSLog(@"CPMyLongContractVC requestCancelContractByID responseObject:%@", responseObject);
        WS(weakSelf);
        if (responseObject) {
            ...
            NSLog(@"CPMyLongContractVC requestCancelContractByID 失败");
        }
        
        
    } failure:^(NSError *error, ParamtersJudgeCode judgeCode) {
        [SVProgressHUD showErrorWithStatus:[error localizedDescription]];
        NSLog(@"CPMyLongContractVC requestCancelContractByID error:%@, judgeCode:%lu", [error localizedDescription], (unsigned long)judgeCode);
    }];
}
...
- (void)requestContractOnCarById:(NSUInteger)index{
    ...
    [[NetworkManager shareNetworkManager] POSTUrl:[NSString stringWithFormat:@"%@/api/contract/v1/confirmOnCar.json", BaseURL] parameters:@{@"contractId":[NSNumber numberWithInteger:contractMJModel.dataid], @"ridingStatus":[NSNumber numberWithInteger:1]}.mutableCopy success:^(id responseObject) {
        [weakSelf.tableView.mj_footer endRefreshing];
        NSLog(@"CPMyLongContractVC requestContractOnCarByIndex responseObject:%@", responseObject);
        if (responseObject) {
            if ([[responseObject valueForKey:@"code"] integerValue] == 200) {
            ...
            NSLog(@"CPMyLongContractVC requestContractOnCarByIndex requestRegister 失败");
        }
        
        
    } failure:^(NSError *error, ParamtersJudgeCode judgeCode) {
        [weakSelf.tableView.mj_footer endRefreshing];
        NSLog(@"CPMyLongContractVC  requestContractOnCarByIndex error:%@, judgeCode:%lu", [error localizedDescription], (unsigned long)judgeCode);
    }];
}
...
- (void)requestConfirmArriveById:(NSUInteger)index{
    ...
    CPContractMJModel *contractMJModel = [self.dataSource objectAtIndex:index];
    NSLog(@"CPMyLongContractVC requestConfirmArriveById contractMJModel.ridingStatus:%ld", (long)contractMJModel.ridingStatus);
    ...
    [[NetworkManager shareNetworkManager] POSTUrl:[NSString stringWithFormat:@"%@/api/contract/v1/confirmArrive", BaseURL] parameters:@{@"contractId":[NSNumber numberWithInteger:contractMJModel.dataid]}.mutableCopy success:^(id responseObject) {
        [SVProgressHUD dismiss];
        NSLog(@"CPMyLongContractVC requestConfirmArriveById responseObject:%@", responseObject);
        WS(weakSelf);
        if (responseObject) {
            if ([[responseObject valueForKey:@"code"] integerValue] == 200) {
            ...
            NSLog(@"CPMyLongContractVC requestConfirmArriveById 失败");
        }
        
        
    } failure:^(NSError *error, ParamtersJudgeCode judgeCode) {
        [SVProgressHUD showErrorWithStatus:[error localizedDescription]];
        NSLog(@"CPMyLongContractVC requestConfirmArriveById error:%@, judgeCode:%lu", [error localizedDescription], (unsigned long)judgeCode);
    }];
}
...
- (void)requestIfUserIsLogin{
    ...
    [[NetworkManager shareNetworkManager] POSTUrl:[NSString stringWithFormat:@"%@/api/auth/v1/islogin", BaseURL] parameters:nil success:^(id responseObject) {
        [SVProgressHUD dismiss];
        NSLog(@"CPMyLongContractVC requestIfUserIsLogin responseObject:%@", responseObject);
        if (responseObject) {
            if ([[responseObject valueForKey:@"code"] integerValue] == 200) {
                NSString *account = [[NSUserDefaults standardUserDefaults] valueForKey:kUserLoginAccount];
                NSLog(@"CPMyLongContractVC requestIfUserIsLogin 拼车号:%@ has login ", account);
                
                NSString *savedToken = [[NSUserDefaults standardUserDefaults] stringForKey:@"savedToken"];
                NSString *savedUserId = [[NSUserDefaults standardUserDefaults] stringForKey:@"savedUserId"];
                
                NSLog(@"CPMyLongContractVC savedUserId:%@, [WFCCNetworkService sharedInstance].userId:%@", savedUserId, [WFCCNetworkService sharedInstance].userId);
            ...
            NSLog(@"CPMyLongContractVC requestIfUserIsLogin 失败");
        }
        
    } failure:^(NSError *error, ParamtersJudgeCode judgeCode) {
        [SVProgressHUD showErrorWithStatus:[error localizedDescription]];
        NSLog(@"CPMyLongContractVC requestIfUserIsLogin error:%@, judgeCode:%lu", [error localizedDescription], (unsigned long)judgeCode);
    }];
}

       ...
       failure:(void (^) (NSError *error, ParamtersJudgeCode  judgeCode))failure
{
    ...
    if (nil != [[NSUserDefaults standardUserDefaults] valueForKey:kUserToken]) {
        NSString *token = [[NSUserDefaults standardUserDefaults] valueForKey:kUserToken];
        [_sessionManager.requestSerializer setValue:token forHTTPHeaderField:@"X-Nideshop-Token"];
    }
    ...
    if (nil != [[NSUserDefaults standardUserDefaults] valueForKey:kUserID]) {
        ...
        [parameters setValue:[NSNumber numberWithLongLong:userId] forKey:@"userId"];
    }
        ...
        failure:(void (^) (NSError *error, ParamtersJudgeCode  judgeCode))failure
{
    ...
    NSLog(@"NetworkManager usertoken:%@", [[NSUserDefaults standardUserDefaults] valueForKey:kUserToken]);
    if (nil != [[NSUserDefaults standardUserDefaults] valueForKey:kUserToken]) {
        NSString *token = [[NSUserDefaults standardUserDefaults] valueForKey:kUserToken];
        [_sessionManager.requestSerializer setValue:token forHTTPHeaderField:@"X-Nideshop-Token"];
    }
    
    NSLog(@"NetworkManager kUserID:%ld", (long)[[[NSUserDefaults standardUserDefaults] valueForKey:kUserID] longLongValue]);
    if (nil != [[NSUserDefaults standardUserDefaults] valueForKey:kUserID]) {
        long long userId = [[[NSUserDefaults standardUserDefaults] valueForKey:kUserID] longLongValue];
        [parameters setValue:[NSNumber numberWithLongLong:userId] forKey:@"userId"];
    }
...
/**
 ...
 @param failure 失败返回
 */
- (void)UploadImageWithUrl:(NSString *)url parameters:(id)parameters
                   ...
                   failure:(void (^) (NSError *error, ParamtersJudgeCode  judgeCode))failure
{
    ...
    //传入参数为空
    if ([self isNullToDictionary:parameters]) {

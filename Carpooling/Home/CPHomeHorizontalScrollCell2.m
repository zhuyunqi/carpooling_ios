...
@interface CPHomeHorizontalScrollCell2 ()<HorizonItemCellDelegate>
@property (weak, nonatomic) IBOutlet UICollectionView *collectionView;
@property (weak, nonatomic) IBOutlet UICollectionViewFlowLayout *layout;
@end

@implementation CPHomeHorizontalScrollCell2{
...
- (void)configCollectionView
{
    ...
    //设置单元格大小
    self.layout.itemSize = CGSizeMake(itemWidth, 165);
}
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.delegate = self;
    cell.scheduleModel = [self.itemsArray objectAtIndex:indexPath.row];
    return cell;
}
...
{
    NSLog(@"HorizontalScrollCell indexPath.row:%ld", indexPath.row);
    if (self.delegate && [self.delegate respondsToSelector:@selector(horizontalScrollCell2ClickIndexPath:)]) {
        [self.delegate horizontalScrollCell2ClickIndexPath:indexPath];
    }
}
...
- (void)horizonItemCellMatchingByIndexPath:(NSIndexPath *)indexPath{
    if (self.delegate && [self.delegate respondsToSelector:@selector(horizontalScrollCell2MatchingByIndexPath:)]) {
        [self.delegate horizontalScrollCell2MatchingByIndexPath:indexPath];
    }
}
...
- (void)horizonItemCellNaviAction:(NSIndexPath *)indexPath location:(CLLocationCoordinate2D)coordinate destination:(NSString *)destination{
    if (self.delegate && [self.delegate respondsToSelector:@selector(horizontalScrollCell2NaviAction:location:destination:)]) {
        [self.delegate horizontalScrollCell2NaviAction:indexPath location:coordinate destination:destination];
    }
}
@end
